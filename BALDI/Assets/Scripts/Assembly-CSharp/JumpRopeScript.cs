...
public class JumpRopeScript : MonoBehaviour
{
	...
	{
		maxJump = UnityEngine.Random.Range(2, 10);
        this.jumpDelay = 1f;
		this.ropeHit = true;
		this.jumpStarted = false;
		this.jumps = 0;
		this.jumpCount.text = "0/" + maxJump;
		this.cs.jumpHeight = 0f;
        Audio.SetRandomPitch(playtime.audioDevice);
	...
	private void Success()
	{
        ...
        Audio.SetRandomPitch(playtime.audioDevice);
        this.playtime.audioDevice.PlayOneShot(this.playtime.aud_Numbers[UnityEngine.Random.Range(0, playtime.aud_Numbers.Length)]);
		this.jumps++;
		this.jumpCount.text = UnityEngine.Random.Range(0, maxJump) + "/" + maxJump;
		this.jumpDelay = 0.5f;
		if (this.jumps >= maxJump) //If players complete the minigame
		{
